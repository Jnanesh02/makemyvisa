...
import "./countriesserviceDetails.css";
export const CountriesServiceDetails = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios(`${process.env.REACT_APP_ADMIN_COUNTRIES}/getcountries`);
                const filteredCountries = response.data.message.filter(country => country.countryName === countryName);
                setCountryData(filteredCountries.length > 0 ? filteredCountries[0] : null,serviceType);
